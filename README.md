...
2. **Node.js 16+** (for the playground proxy server)
...
### Step 3: Start Playground Server
...
**Note:** The proxy server solves CORS issues by forwarding API requests from `/api/*` to `http://127.0.0.1:8080/*` on the same origin.
...
If you prefer the old static server (may have CORS issues):

```bash
./scripts/run_static_server.sh
# Then manually set Base URL to http://127.0.0.1:8080 in the playground
```
